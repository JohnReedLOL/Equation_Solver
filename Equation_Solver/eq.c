...
int main()
{
	...
	exit(EXIT_FAILURE);
    printf("Gcc is compiling your equation...n");
    i = system("gcc -march=native -Ofast -Wall -Wextra -pedantic -Werror ./eq_helper.c -o helper_function -lm");
    if (i == 0) {
	printf("gcc compilation returned successful.n");
	i = system("./helper_function");
	if (i == 0) {
	    printf("success!n");
	    return 0;
	} else {
	    perror("helper function process returned an error code");
	    return 1;
	}
    } else {
